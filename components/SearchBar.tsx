    ...
    return (
        <View className={`relative flex-row items-center px-5 bg-dark-200 rounded-full focus-within:scale-105 transition-transform duration-200 ${className}`}>
            ...
            <TextInput
                ...
                onChange={() => { }} placeholderTextColor="#ab8bff" className='flex-1 ml-2 py-3 text-white outline-none'
            />
            {canClearValue && (
                <Ionicons name="close-outline" size={20} color="#ab8bff" className="absolute right-4 top-1/2 -translate-y-1/2"
                    onPress={clearValue}
                />
            )}
        </View>
